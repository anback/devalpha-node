...
it('createStreamMerged returns a merged stream of Redux actions', done => {
  const streams = {
    foo: _(['FOO']),
    bar: _(['BAR'])
  }
...
it('createStreamMerged runs event in arbitrary order', done => {
  let i1
  let i2
...
it('createStreamSorted returns a sorted stream of Redux actions', done => {
  const streams = {
    foo: _([{ timestamp: 10 }]),
    bar: _([{ timestamp: 5 }]),
    baz: _([{}]),
...
it('createStreamSorted handles errors', done => {
  const streams = {
